...
export class IconResolver {
	...
	private _getIconUrl(iconName: string): string {
	...
	/**
	 * Get the URL for a folder icon based on its name.
	...
	folder(name: string, expanded: boolean = false): string {
		if (!expanded && name in this._config.folderNames) {
			return this._getIconUrl(this._config.folderNames[name]);
		}
		if (expanded && name in this._config.folderNamesExpanded) {
			return this._getIconUrl(this._config.folderNamesExpanded[name]);
		}
		return this._getIconUrl(expanded ? this._config.folderExpanded : this._config.folder);
	}

	/**
	 * Get the URL for a file icon based on its name and optionally language type.
	...
	file(name: string, language: string = ''): string {
		if (name in this._config.fileNames) {
			return this._getIconUrl(this._config.fileNames[name]);
		}
		if (language && language in this._config.languageIds) {
			return this._getIconUrl(this._config.languageIds[language]);
		}
		...
		while (extensions.length) {
			...
			if (current in this._config.fileExtensions) {
				return this._getIconUrl(this._config.fileExtensions[current]);
			}
			extensions.shift();
		}
		return this._getIconUrl(this._config.file);
	}
}
 ...
 */
export async function load(filename: string): Promise<IconResolver> {
	const iconJson = await (await request(filename)).json<IconJson>();
	return new IconResolver(filename, iconJson);
}
